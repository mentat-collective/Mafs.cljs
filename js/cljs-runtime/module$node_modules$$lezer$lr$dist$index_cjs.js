...
function readToken(data,input,stack,group,precTable,precOffset){var state=0;group=1<<group;({dialect:stack}=stack.p.parser);a:for(;0!=(group&data[state]);){let accEnd=data[state+1];for(var i=state+3;i<accEnd;i+=2)if(0<(data[i+1]&group)){var term=data[i],JSCompiler_temp;if((JSCompiler_temp=stack.allows(term))&&!(JSCompiler_temp=-1==input.token.value||input.token.value==term)){JSCompiler_temp=term;var tableData=precTable,tableOffset=precOffset;let iPrev=findOffset(tableData,tableOffset,input.token.value);
JSCompiler_temp=0>iPrev||findOffset(tableData,tableOffset,JSCompiler_temp)<iPrev}if(JSCompiler_temp){input.acceptToken(term);break}}i=input.next;term=0;state=data[state+2];if(0>input.next&&state>term&&65535==data[accEnd+3*state-3]&&65535==data[accEnd+3*state-3]){state=data[accEnd+3*state-1];continue a}for(;term<state;)if(JSCompiler_temp=term+state>>1,tableData=accEnd+JSCompiler_temp+(JSCompiler_temp<<1),tableOffset=data[tableData+1]||65536,i<data[tableData])state=JSCompiler_temp;else if(i>=tableOffset)term=
JSCompiler_temp+1;else{state=data[tableData+2];input.advance();continue a}break}}function findOffset(data,start,term){for(let i=start,next;65535!=(next=data[i]);i++)if(next==term)return i-start;return-1}function cutAt(tree,pos,side){let cursor=tree.cursor(common.IterMode.IncludeAnonymous);for(cursor.moveTo(pos);;)if(0>side?!cursor.childBefore(pos):!cursor.childAfter(pos))for(;;){if((0>side?cursor.to<pos:cursor.from>pos)&&!cursor.type.isError)return 0>side?Math.max(0,Math.min(cursor.to-1,pos-25)):
Math.min(tree.length,Math.max(cursor.from+1,pos+25));if(0>side?cursor.prevSibling():cursor.nextSibling())break;if(!cursor.parent())return 0>side?0:tree.length}}function pushStackDedup(stack,newStacks){for(let i=0;i<newStacks.length;i++){let other=newStacks[i];if(other.pos==stack.pos&&other.sameState(stack)){newStacks[i].score<stack.score&&(newStacks[i]=stack);return}}newStacks.push(stack)}function pair(data,off){return data[off]|data[off+1]<<16}function findFinished(stacks){let best=null;for(let stack of stacks)stacks=
stack.p.stoppedAt,(stack.pos==stack.p.stream.end||null!=stacks&&stack.pos>stacks)&&stack.p.parser.stateFlag(stack.state,2)&&(!best||best.score<stack.score)&&(best=stack);return best}function getSpecializer(spec){if(spec.external){let mask=spec.extend?1:0;return(value,stack)=>spec.external(value,stack)<<1|mask}return spec.get}global=require("module$node_modules$process$browser");"use strict";Object.defineProperty(exports,"__esModule",{value:!0});var common=require("module$node_modules$$lezer$common$dist$index_cjs");
class Stack{constructor(p,stack,state,reducePos,pos,score,buffer,bufferBase,curContext,lookAhead=0,parent){this.p=p;this.stack=stack;this.state=state;this.reducePos=reducePos;this.pos=pos;this.score=score;this.buffer=buffer;this.bufferBase=bufferBase;this.curContext=curContext;this.lookAhead=lookAhead;this.parent=parent}toString(){return`[${this.stack.filter((_,i)=>0==i%3).concat(this.state)}]@${this.pos}${this.score?"!"+this.score:""}`}static start(p,state,pos=0){let cx=p.parser.context;return new Stack(p,
[],state,pos,pos,0,[],0,cx?new StackContext(cx,cx.start):null,0,null)}get context(){return this.curContext?this.curContext.context:null}pushState(state,start){this.stack.push(this.state,start,this.bufferBase+this.buffer.length);this.state=state}reduce(action){var depth=action>>19;let type=action&65535,{parser}=this.p;var dPrec=parser.dynamicPrecedence(type);dPrec&&(this.score+=dPrec);if(0==depth)this.pushState(parser.getGoto(this.state,type,!0),this.reducePos),type<parser.minRepeatTerm&&this.storeNode(type,
this.reducePos,this.reducePos,4,!0),this.reduceContext(type,this.reducePos);else{dPrec=(depth=this.stack.length-3*(depth-1)-(action&262144?6:0))?this.stack[depth-2]:0;var size=this.reducePos-dPrec;2E3<=size&&(dPrec==this.p.lastBigReductionStart?(this.p.bigReductionCount++,this.p.lastBigReductionSize=size):this.p.lastBigReductionSize<size&&(this.p.bigReductionCount=1,this.p.lastBigReductionStart=dPrec,this.p.lastBigReductionSize=size));size=this.bufferBase+this.buffer.length-(depth?this.stack[depth-
1]:0);if(type<parser.minRepeatTerm||action&131072){let pos=parser.stateFlag(this.state,1)?this.pos:this.reducePos;this.storeNode(type,dPrec,pos,size+4,!0)}for(this.state=action&262144?this.stack[depth]:parser.getGoto(this.stack[depth-3],type,!0);this.stack.length>depth;)this.stack.pop();this.reduceContext(type,dPrec)}}storeNode(term,start,end,size=4,isReduce=!1){if(0==term&&(!this.stack.length||this.stack[this.stack.length-1]<this.buffer.length+this.bufferBase)){let cur=this,top=this.buffer.length;
0==top&&cur.parent&&(top=cur.bufferBase-cur.parent.bufferBase,cur=cur.parent);if(0<top&&0==cur.buffer[top-4]&&-1<cur.buffer[top-1]){if(start==end)return;if(cur.buffer[top-2]>=start){cur.buffer[top-2]=end;return}}}if(isReduce&&this.pos!=end){isReduce=this.buffer.length;if(0<isReduce&&0!=this.buffer[isReduce-4])for(;0<isReduce&&this.buffer[isReduce-2]>end;)this.buffer[isReduce]=this.buffer[isReduce-4],this.buffer[isReduce+1]=this.buffer[isReduce-3],this.buffer[isReduce+2]=this.buffer[isReduce-2],this.buffer[isReduce+
3]=this.buffer[isReduce-1],isReduce-=4,4<size&&(size-=4);this.buffer[isReduce]=term;this.buffer[isReduce+1]=start;this.buffer[isReduce+2]=end;this.buffer[isReduce+3]=size}else this.buffer.push(term,start,end,size)}shift(action,next,nextEnd){let start=this.pos;if(action&131072)this.pushState(action&65535,this.pos);else if(0==(action&262144)){let {parser}=this.p;if(nextEnd>this.pos||next<=parser.maxNode)this.pos=nextEnd,parser.stateFlag(action,1)||(this.reducePos=nextEnd);this.pushState(action,start);
this.shiftContext(next,start);next<=parser.maxNode&&this.buffer.push(next,start,nextEnd,4)}else this.pos=nextEnd,this.shiftContext(next,start),next<=this.p.parser.maxNode&&this.buffer.push(next,start,nextEnd,4)}apply(action,next,nextEnd){action&65536?this.reduce(action):this.shift(action,next,nextEnd)}useNode(value,next){let index=this.p.reused.length-1;if(0>index||this.p.reused[index]!=value)this.p.reused.push(value),index++;let start=this.pos;this.reducePos=this.pos=start+value.length;this.pushState(next,
start);this.buffer.push(index,start,this.reducePos,-1);this.curContext&&this.updateContext(this.curContext.tracker.reuse(this.curContext.context,value,this,this.p.stream.reset(this.pos-value.length)))}split(){let parent=this;for(var off=parent.buffer.length;0<off&&parent.buffer[off-2]>parent.reducePos;)off-=4;let buffer=parent.buffer.slice(off);for(off=parent.bufferBase+off;parent&&off==parent.bufferBase;)parent=parent.parent;return new Stack(this.p,this.stack.slice(),this.state,this.reducePos,this.pos,
this.score,buffer,off,this.curContext,this.lookAhead,parent)}recoverByDelete(next,nextEnd){let isNode=next<=this.p.parser.maxNode;isNode&&this.storeNode(next,this.pos,nextEnd,4);this.storeNode(0,this.pos,nextEnd,isNode?8:4);this.pos=this.reducePos=nextEnd;this.score-=190}canShift(term){for(let sim=new SimulatedStack(this);;){let action=this.p.parser.stateSlot(sim.state,4)||this.p.parser.hasAction(sim.state,term);if(0==action)return!1;if(0==(action&65536))return!0;sim.reduce(action)}}recoverByInsert(next){if(300<=
this.stack.length)return[];let nextStates=this.p.parser.nextStates(this.state);if(8<nextStates.length||120<=this.stack.length){var best=[];for(let i=0,s;i<nextStates.length;i+=2)(s=nextStates[i+1])!=this.state&&this.p.parser.hasAction(s,next)&&best.push(nextStates[i],s);if(120>this.stack.length)for(next=0;8>best.length&&next<nextStates.length;next+=2){let s=nextStates[next+1];best.some((v,i)=>i&1&&v==s)||best.push(nextStates[next],s)}nextStates=best}best=[];for(next=0;next<nextStates.length&&4>best.length;next+=
2){let s=nextStates[next+1];if(s==this.state)continue;let stack=this.split();stack.pushState(s,this.pos);stack.storeNode(0,stack.pos,stack.pos,4,!0);stack.shiftContext(nextStates[next],this.pos);stack.score-=200;best.push(stack)}return best}forceReduce(){let reduce=this.p.parser.stateSlot(this.state,5);if(0==(reduce&65536))return!1;let {parser}=this.p;if(!parser.validAction(this.state,reduce)){let target=this.stack.length-3*(reduce>>19);if(0>target||0>parser.getGoto(this.stack[target],reduce&65535,
!1))return!1;this.storeNode(0,this.reducePos,this.reducePos,4,!0);this.score-=100}this.reducePos=this.pos;this.reduce(reduce);return!0}forceAll(){for(;!this.p.parser.stateFlag(this.state,2);)if(!this.forceReduce()){this.storeNode(0,this.pos,this.pos,4,!0);break}return this}get deadEnd(){if(3!=this.stack.length)return!1;let {parser}=this.p;return 65535==parser.data[parser.stateSlot(this.state,1)]&&!parser.stateSlot(this.state,4)}restart(){this.state=this.stack[0];this.stack.length=0}sameState(other){if(this.state!=
other.state||this.stack.length!=other.stack.length)return!1;for(let i=0;i<this.stack.length;i+=3)if(this.stack[i]!=other.stack[i])return!1;return!0}get parser(){return this.p.parser}dialectEnabled(dialectID){return this.p.parser.dialect.flags[dialectID]}shiftContext(term,start){this.curContext&&this.updateContext(this.curContext.tracker.shift(this.curContext.context,term,this,this.p.stream.reset(start)))}reduceContext(term,start){this.curContext&&this.updateContext(this.curContext.tracker.reduce(this.curContext.context,
term,this,this.p.stream.reset(start)))}emitContext(){let last=this.buffer.length-1;(0>last||-3!=this.buffer[last])&&this.buffer.push(this.curContext.hash,this.reducePos,this.reducePos,-3)}emitLookAhead(){let last=this.buffer.length-1;(0>last||-4!=this.buffer[last])&&this.buffer.push(this.lookAhead,this.reducePos,this.reducePos,-4)}updateContext(context){context!=this.curContext.context&&(context=new StackContext(this.curContext.tracker,context),context.hash!=this.curContext.hash&&this.emitContext(),
this.curContext=context)}setLookAhead(lookAhead){lookAhead>this.lookAhead&&(this.emitLookAhead(),this.lookAhead=lookAhead)}close(){this.curContext&&this.curContext.tracker.strict&&this.emitContext();0<this.lookAhead&&this.emitLookAhead()}}class StackContext{constructor(tracker,context){this.tracker=tracker;this.context=context;this.hash=tracker.strict?tracker.hash(context):0}}var Recover;(function(Recover){Recover[Recover.Insert=200]="Insert";Recover[Recover.Delete=190]="Delete";Recover[Recover.Reduce=
100]="Reduce";Recover[Recover.MaxNext=4]="MaxNext";Recover[Recover.MaxInsertStackDepth=300]="MaxInsertStackDepth";Recover[Recover.DampenInsertStackDepth=120]="DampenInsertStackDepth";Recover[Recover.MinBigReduction=2E3]="MinBigReduction"})(Recover||(Recover={}));class SimulatedStack{constructor(start){this.start=start;this.state=start.state;this.stack=start.stack;this.base=this.stack.length}reduce(action){let term=action&65535;action>>=19;0==action?(this.stack==this.start.stack&&(this.stack=this.stack.slice()),
...
next.bufferBase,this.stack=next,this.buffer=next.buffer)}get id(){return this.buffer[this.index-4]}get start(){return this.buffer[this.index-3]}get end(){return this.buffer[this.index-2]}get size(){return this.buffer[this.index-1]}next(){this.index-=4;this.pos-=4;0==this.index&&this.maybeNext()}fork(){return new StackBufferCursor(this.stack,this.pos,this.index)}}class CachedToken{constructor(){this.extended=this.end=this.value=this.start=-1;this.context=this.mask=this.lookAhead=0}}const nullToken=
new CachedToken;class InputStream{constructor(input,ranges){this.input=input;this.ranges=ranges;this.chunk="";this.chunkOff=0;this.chunk2="";this.chunk2Pos=0;this.next=-1;this.token=nullToken;this.rangeIndex=0;this.pos=this.chunkPos=ranges[0].from;this.range=ranges[0];this.end=ranges[ranges.length-1].to;this.readNext()}resolveOffset(offset,assoc){let range=this.range,index=this.rangeIndex;for(offset=this.pos+offset;offset<range.from;){if(!index)return null;var next=this.ranges[--index];offset-=range.from-
next.to;range=next}for(;0>assoc?offset>range.to:offset>=range.to;){if(index==this.ranges.length-1)return null;next=this.ranges[++index];offset+=next.from-range.to;range=next}return offset}clipPos(pos){if(pos>=this.range.from&&pos<this.range.to)return pos;for(let range of this.ranges)if(range.to>pos)return Math.max(pos,range.from);return this.end}peek(offset){var idx=this.chunkOff+offset;if(0<=idx&&idx<this.chunk.length)offset=this.pos+offset,idx=this.chunk.charCodeAt(idx);else{offset=this.resolveOffset(offset,
1);if(null==offset)return-1;if(offset>=this.chunk2Pos&&offset<this.chunk2Pos+this.chunk2.length)idx=this.chunk2.charCodeAt(offset-this.chunk2Pos);else{idx=this.rangeIndex;let range=this.range;for(;range.to<=offset;)range=this.ranges[++idx];this.chunk2=this.input.chunk(this.chunk2Pos=offset);offset+this.chunk2.length>range.to&&(this.chunk2=this.chunk2.slice(0,range.to-offset));idx=this.chunk2.charCodeAt(0)}}offset>=this.token.lookAhead&&(this.token.lookAhead=offset+1);return idx}acceptToken(token,
endOffset=0){endOffset=endOffset?this.resolveOffset(endOffset,-1):this.pos;if(null==endOffset||endOffset<this.token.start)throw new RangeError("Token end out of bounds");this.token.value=token;this.token.end=endOffset}getChunk(){if(this.pos>=this.chunk2Pos&&this.pos<this.chunk2Pos+this.chunk2.length){let {chunk,chunkPos}=this;this.chunk=this.chunk2;this.chunkPos=this.chunk2Pos;this.chunk2=chunk;this.chunk2Pos=chunkPos;this.chunkOff=this.pos-this.chunkPos}else{this.chunk2=this.chunk;this.chunk2Pos=
this.chunkPos;let nextChunk=this.input.chunk(this.pos);this.chunk=this.pos+nextChunk.length>this.range.to?nextChunk.slice(0,this.range.to-this.pos):nextChunk;this.chunkPos=this.pos;this.chunkOff=0}}readNext(){return this.chunkOff>=this.chunk.length&&(this.getChunk(),this.chunkOff==this.chunk.length)?this.next=-1:this.next=this.chunk.charCodeAt(this.chunkOff)}advance(n=1){for(this.chunkOff+=n;this.pos+n>=this.range.to;){if(this.rangeIndex==this.ranges.length-1)return this.setDone();n-=this.range.to-
this.pos;this.range=this.ranges[++this.rangeIndex];this.pos=this.range.from}this.pos+=n;this.pos>=this.token.lookAhead&&(this.token.lookAhead=this.pos+1);return this.readNext()}setDone(){this.pos=this.chunkPos=this.end;this.range=this.ranges[this.rangeIndex=this.ranges.length-1];this.chunk="";return this.next=-1}reset(pos,token){token?(this.token=token,token.start=pos,token.lookAhead=pos+1,token.value=token.extended=-1):this.token=nullToken;if(this.pos!=pos){this.pos=pos;if(pos==this.end)return this.setDone(),
...
this.chunk2Pos,to-this.chunk2Pos);if(from>=this.range.from&&to<=this.range.to)return this.input.read(from,to);let result="";for(let r of this.ranges){if(r.from>=to)break;r.to>from&&(result+=this.input.read(Math.max(r.from,from),Math.min(r.to,to)))}return result}}class TokenGroup{constructor(data,id){this.data=data;this.id=id}token(input,stack){let {parser}=stack.p;readToken(this.data,input,stack,this.id,parser.data,parser.tokenPrecTable)}}TokenGroup.prototype.contextual=TokenGroup.prototype.fallback=
TokenGroup.prototype.extend=!1;class LocalTokenGroup{constructor(data,precTable,elseToken){this.precTable=precTable;this.elseToken=elseToken;this.data="string"==typeof data?decodeArray(data):data}token(input,stack){let start=input.pos,cur;for(;;){cur=input.pos;readToken(this.data,input,stack,0,this.data,this.precTable);if(-1<input.token.value)break;if(null==this.elseToken)return;if(0>input.next)break;input.advance();input.reset(cur+1,input.token)}cur>start&&(input.reset(start,input.token),input.acceptToken(this.elseToken,
cur-start))}}LocalTokenGroup.prototype.contextual=TokenGroup.prototype.fallback=TokenGroup.prototype.extend=!1;class ExternalTokenizer{constructor(token,options={}){this.token=token;this.contextual=!!options.contextual;this.fallback=!!options.fallback;this.extend=!!options.extend}}const verbose="undefined"!=typeof global&&global.env&&/\bparse\b/.test(global.env.LOG);let stackIDs=null;var Safety;(function(Safety){Safety[Safety.Margin=25]="Margin"})(Safety||(Safety={}));class FragmentCursor{constructor(fragments,
nodeSet){this.fragments=fragments;this.nodeSet=nodeSet;this.i=0;this.fragment=null;this.safeTo=this.safeFrom=-1;this.trees=[];this.start=[];this.index=[];this.nextFragment()}nextFragment(){let fr=this.fragment=this.i==this.fragments.length?null:this.fragments[this.i++];if(fr){this.safeFrom=fr.openStart?cutAt(fr.tree,fr.from+fr.offset,1)-fr.offset:fr.from;for(this.safeTo=fr.openEnd?cutAt(fr.tree,fr.to+fr.offset,-1)-fr.offset:fr.to;this.trees.length;)this.trees.pop(),this.start.pop(),this.index.pop();
this.trees.push(fr.tree);this.start.push(-fr.offset);this.index.push(0);this.nextStart=this.safeFrom}else this.nextStart=1E9}nodeAt(pos){if(pos<this.nextStart)return null;for(;this.fragment&&this.safeTo<=pos;)this.nextFragment();if(!this.fragment)return null;for(;;){let last=this.trees.length-1;if(0>last)return this.nextFragment(),null;var top=this.trees[last],index=this.index[last];if(index==top.children.length){this.trees.pop();this.start.pop();this.index.pop();continue}let next=top.children[index];
top=this.start[last]+top.positions[index];if(top>pos)return this.nextStart=top,null;if(next instanceof common.Tree){if(top==pos){if(top<this.safeFrom)return null;index=top+next.length;if(index<=this.safeTo){let lookAhead=next.prop(common.NodeProp.lookAhead);if(!lookAhead||index+lookAhead<this.fragment.to)return next}}this.index[last]++;top+next.length>=Math.max(this.safeFrom,pos)&&(this.trees.push(next),this.start.push(top),this.index.push(0))}else this.index[last]++,this.nextStart=top+next.length}}}
class TokenCache{constructor(parser,stream){this.stream=stream;this.tokens=[];this.mainToken=null;this.actions=[];this.tokens=parser.tokenizers.map(_=>new CachedToken)}getActions(stack){let actionIndex=0,main=null;var {parser}=stack.p;let {tokenizers}=parser;parser=parser.stateSlot(stack.state,3);let context=stack.curContext?stack.curContext.hash:0,lookAhead=0;for(let i=0;i<tokenizers.length;i++){if(0==(1<<i&parser))continue;let tokenizer=tokenizers[i],token=this.tokens[i];if(!main||tokenizer.fallback){if(tokenizer.contextual||
token.start!=stack.pos||token.mask!=parser||token.context!=context)this.updateCachedToken(token,tokenizer,stack),token.mask=parser,token.context=context;token.lookAhead>token.end+25&&(lookAhead=Math.max(token.lookAhead,lookAhead));if(0!=token.value){let startIndex=actionIndex;-1<token.extended&&(actionIndex=this.addActions(stack,token.extended,token.end,actionIndex));actionIndex=this.addActions(stack,token.value,token.end,actionIndex);if(!tokenizer.extend&&(main=token,actionIndex>startIndex))break}}}for(;this.actions.length>
actionIndex;)this.actions.pop();lookAhead&&stack.setLookAhead(lookAhead);main||stack.pos!=this.stream.end||(main=new CachedToken,main.value=stack.p.parser.eofTerm,main.start=main.end=stack.pos,this.addActions(stack,main.value,main.end,actionIndex));this.mainToken=main;return this.actions}getMainToken(stack){if(this.mainToken)return this.mainToken;let main=new CachedToken,{pos,p}=stack;main.start=pos;main.end=Math.min(pos+1,p.stream.end);main.value=pos==p.stream.end?p.parser.eofTerm:0;return main}updateCachedToken(token,
tokenizer,stack){var start=this.stream.clipPos(stack.pos);tokenizer.token(this.stream.reset(start,token),stack);if(-1<token.value)for({parser:tokenizer}=stack.p,start=0;start<tokenizer.specialized.length;start++){if(tokenizer.specialized[start]==token.value){let result=tokenizer.specializers[start](this.stream.read(token.start,token.end),stack);if(0<=result&&stack.p.parser.dialect.allows(result>>1)){0==(result&1)?token.value=result>>1:token.extended=result>>1;break}}}else token.value=0,token.end=
this.stream.clipPos(start+1)}putAction(action,token,end,index){for(let i=0;i<index;i+=3)if(this.actions[i]==action)return index;this.actions[index++]=action;this.actions[index++]=token;this.actions[index++]=end;return index}addActions(stack,token,end,index){let {state}=stack;({parser:stack}=stack.p);let {data}=stack;for(let set=0;2>set;set++)for(let i=stack.stateSlot(state,set?2:1);;i+=3){if(65535==data[i])if(1==data[i+1])i=pair(data,i+2);else{0==index&&2==data[i+1]&&(index=this.putAction(pair(data,
i+2),token,end,index));break}data[i]==token&&(index=this.putAction(pair(data,i+1),token,end,index))}return index}}var Rec;(function(Rec){Rec[Rec.Distance=5]="Distance";Rec[Rec.MaxRemainingPerStep=3]="MaxRemainingPerStep";Rec[Rec.MinBufferLengthPrune=500]="MinBufferLengthPrune";Rec[Rec.ForceReduceLimit=10]="ForceReduceLimit";Rec[Rec.CutDepth=15E3]="CutDepth";Rec[Rec.CutTo=9E3]="CutTo";Rec[Rec.MaxLeftAssociativeReductionCount=1E3]="MaxLeftAssociativeReductionCount"})(Rec||(Rec={}));class Parse{constructor(parser,
input,fragments,ranges){this.parser=parser;this.input=input;this.ranges=ranges;this.recovering=0;this.nextStackID=9812;this.minStackPos=0;this.reused=[];this.stoppedAt=null;this.lastBigReductionStart=-1;this.bigReductionCount=this.lastBigReductionSize=0;this.stream=new InputStream(input,ranges);this.tokens=new TokenCache(parser,this.stream);this.topTerm=parser.top[1];({from:input}=ranges[0]);this.stacks=[Stack.start(this,parser.top[0],input)];this.fragments=fragments.length&&this.stream.end-input>
4*parser.bufferLength?new FragmentCursor(fragments,parser.nodeSet):null}get parsedPos(){return this.minStackPos}advance(){var stacks=this.stacks;let pos=this.minStackPos,newStacks=this.stacks=[];if(1E3<this.bigReductionCount&&1==stacks.length){for(var [s$jscomp$0]=stacks;s$jscomp$0.forceReduce()&&s$jscomp$0.stack.length&&s$jscomp$0.stack[s$jscomp$0.stack.length-2]>=this.lastBigReductionStart;);this.bigReductionCount=this.lastBigReductionSize=0}for(s$jscomp$0=0;s$jscomp$0<stacks.length;s$jscomp$0++)for(var stack=
stacks[s$jscomp$0];;){this.tokens.mainToken=null;if(stack.pos>pos)newStacks.push(stack);else if(this.advanceStack(stack,newStacks,stacks))continue;else{if(!stopped){var stopped=[];var stoppedTokens=[]}stopped.push(stack);stack=this.tokens.getMainToken(stack);stoppedTokens.push(stack.value,stack.end)}break}if(!newStacks.length){if(stacks=stopped&&findFinished(stopped))return this.stackToTree(stacks);if(this.parser.strict)throw verbose&&stopped&&console.log("Stuck with token "+(this.tokens.mainToken?
this.parser.getName(this.tokens.mainToken.value):"none")),new SyntaxError("No parse at "+pos);this.recovering||(this.recovering=5)}if(this.recovering&&stopped&&(stopped=null!=this.stoppedAt&&stopped[0].pos>this.stoppedAt?stopped[0]:this.runRecovery(stopped,stoppedTokens,newStacks)))return this.stackToTree(stopped.forceAll());if(this.recovering){stopped=1==this.recovering?1:3*this.recovering;if(newStacks.length>stopped)for(newStacks.sort((a,b)=>b.score-a.score);newStacks.length>stopped;)newStacks.pop();
newStacks.some(s=>s.reducePos>pos)&&this.recovering--}else if(1<newStacks.length)a:for(stopped=0;stopped<newStacks.length-1;stopped++)for(stoppedTokens=newStacks[stopped],stacks=stopped+1;stacks<newStacks.length;stacks++)if(s$jscomp$0=newStacks[stacks],stoppedTokens.sameState(s$jscomp$0)||500<stoppedTokens.buffer.length&&500<s$jscomp$0.buffer.length)if(0<(stoppedTokens.score-s$jscomp$0.score||stoppedTokens.buffer.length-s$jscomp$0.buffer.length))newStacks.splice(stacks--,1);else{newStacks.splice(stopped--,
1);continue a}this.minStackPos=newStacks[0].pos;for(stopped=1;stopped<newStacks.length;stopped++)newStacks[stopped].pos<this.minStackPos&&(this.minStackPos=newStacks[stopped].pos);return null}stopAt(pos){if(null!=this.stoppedAt&&this.stoppedAt<pos)throw new RangeError("Can't move stoppedAt forward");this.stoppedAt=pos}advanceStack(stack,stacks,split){let start=stack.pos,{parser}=this,base=verbose?this.stackID(stack)+" -\x3e ":"";if(null!=this.stoppedAt&&start>this.stoppedAt)return stack.forceReduce()?
...
1])state=pair(this.data,state+2);else break;if(0==(this.data[state+2]&1)){let value=this.data[state+1];result.some((v,i)=>i&1&&v==value)||result.push(this.data[state],value)}}return result}configure(config){let copy=Object.assign(Object.create(LRParser.prototype),this);config.props&&(copy.nodeSet=this.nodeSet.extend(...config.props));if(config.top){let info=this.topRules[config.top];if(!info)throw new RangeError(`Invalid top rule name ${config.top}`);copy.top=info}config.tokenizers&&(copy.tokenizers=
this.tokenizers.map(t=>{let found=config.tokenizers.find(r=>r.from==t);return found?found.to:t}));config.specializers&&(copy.specializers=this.specializers.slice(),copy.specializerSpecs=this.specializerSpecs.map((s,i)=>{var found=config.specializers.find(r=>r.from==s.external);if(!found)return s;found=Object.assign(Object.assign({},s),{external:found.to});copy.specializers[i]=getSpecializer(found);return found}));config.contextTracker&&(copy.context=config.contextTracker);config.dialect&&(copy.dialect=
this.parseDialect(config.dialect));null!=config.strict&&(copy.strict=config.strict);config.wrap&&(copy.wrappers=copy.wrappers.concat(config.wrap));null!=config.bufferLength&&(copy.bufferLength=config.bufferLength);return copy}hasWrappers(){return 0<this.wrappers.length}getName(term){return this.termNames?this.termNames[term]:String(term<=this.maxNode&&this.nodeSet.types[term].name||term)}get eofTerm(){return this.maxNode+1}get topNode(){return this.nodeSet.types[this.top[1]]}dynamicPrecedence(term){let prec=
this.dynamicPrecedences;return null==prec?0:prec[term]||0}parseDialect(dialect){let values=Object.keys(this.dialects),flags=values.map(()=>!1);if(dialect)for(var part of dialect.split(" ")){var id$jscomp$0=values.indexOf(part);0<=id$jscomp$0&&(flags[id$jscomp$0]=!0)}part=null;for(id$jscomp$0=0;id$jscomp$0<values.length;id$jscomp$0++)if(!flags[id$jscomp$0])for(let j=this.dialects[values[id$jscomp$0]],id;65535!=(id=this.data[j++]);)(part||(part=new Uint8Array(this.maxTerm+1)))[id]=1;return new Dialect(dialect,
flags,part)}static deserialize(spec){return new LRParser(spec)}}exports.ContextTracker=ContextTracker;exports.ExternalTokenizer=ExternalTokenizer;exports.InputStream=InputStream;exports.LRParser=LRParser;exports.LocalTokenGroup=LocalTokenGroup;exports.Stack=Stack}
