...
33554431)<<5)}static range(anchor,head,goalColumn,bidiLevel){goalColumn=(null!==goalColumn&&void 0!==goalColumn?goalColumn:33554431)<<5|(null==bidiLevel?3:Math.min(2,bidiLevel));return head<anchor?SelectionRange.create(head,anchor,24|goalColumn):SelectionRange.create(anchor,head,(head>anchor?4:0)|goalColumn)}static normalized(ranges,mainIndex=0){mainIndex=ranges[mainIndex];ranges.sort((a,b)=>a.from-b.from);mainIndex=ranges.indexOf(mainIndex);for(let i=1;i<ranges.length;i++){let range=ranges[i];var prev=
ranges[i-1];if(range.empty?range.from<=prev.to:range.from<prev.to){let from=prev.from;prev=Math.max(range.to,prev.to);i<=mainIndex&&mainIndex--;ranges.splice(--i,2,range.anchor>range.head?EditorSelection.range(prev,from):EditorSelection.range(from,prev))}}return new EditorSelection(ranges,mainIndex)}}let nextID=0;class Facet{constructor(combine,compareInput,compare,isStatic,enables){this.combine=combine;this.compareInput=compareInput;this.compare=compare;this.isStatic=isStatic;this.id=nextID++;this.default=
combine([]);this.extensions="function"==typeof enables?enables(this):enables}static define(config={}){return new Facet(config.combine||(a=>a),config.compareInput||((a,b)=>a===b),config.compare||(config.combine?(a,b)=>a===b:sameArray),!!config.static,config.enables)}of(value){return new FacetProvider([],this,0,value)}compute(deps,get){if(this.isStatic)throw Error("Can't compute a static facet");return new FacetProvider(deps,this,1,get)}computeN(deps,get){if(this.isStatic)throw Error("Can't compute a static facet");
return new FacetProvider(deps,this,2,get)}from(field,get){get||(get=x=>x);return this.compute([field],state=>get(state.field(field)))}}class FacetProvider{constructor(dependencies,facet,type,value){this.dependencies=dependencies;this.facet=facet;this.type=type;this.value=value;this.id=nextID++}dynamicSlot(addresses){var _a;let getter=this.value,compare=this.facet.compareInput,id=this.id,idx=addresses[id]>>1,multi=2==this.type,depDoc=!1,depSel=!1,depAddrs=[];for(let dep of this.dependencies)"doc"==
dep?depDoc=!0:"selection"==dep?depSel=!0:0==((null!==(_a=addresses[dep.id])&&void 0!==_a?_a:1)&1)&&depAddrs.push(addresses[dep.id]);return{create(state){state.values[idx]=getter(state);return 1},update(state,tr){if(depDoc&&tr.docChanged||depSel&&(tr.docChanged||tr.selection)||ensureAll(state,depAddrs))if(tr=getter(state),multi?!compareArray(tr,state.values[idx],compare):!compare(tr,state.values[idx]))return state.values[idx]=tr,1;return 0},reconfigure:(state,oldState)=>{let newVal;var oldAddr=oldState.config.address[id];
if(null!=oldAddr){if(oldAddr=getAddr(oldState,oldAddr),this.dependencies.every(dep=>dep instanceof Facet?oldState.facet(dep)===state.facet(dep):dep instanceof StateField?oldState.field(dep,!1)==state.field(dep,!1):!0)||(multi?compareArray(newVal=getter(state),oldAddr,compare):compare(newVal=getter(state),oldAddr)))return state.values[idx]=oldAddr,0}else newVal=getter(state);state.values[idx]=newVal;return 1}}}}const initField=Facet.define({static:!0});class StateField{constructor(id,createF,updateF,
compareF,spec){this.id=id;this.createF=createF;this.updateF=updateF;this.compareF=compareF;this.spec=spec;this.provides=void 0}static define(config){let field=new StateField(nextID++,config.create,config.update,config.compare||((a,b)=>a===b),config);config.provide&&(field.provides=config.provide(field));return field}create(state){let init=state.facet(initField).find(i=>i.field==this);return((null===init||void 0===init?void 0:init.create)||this.createF)(state)}slot(addresses){let idx=addresses[this.id]>>
1;return{create:state=>{state.values[idx]=this.create(state);return 1},update:(state,tr)=>{let oldVal=state.values[idx];tr=this.updateF(oldVal,tr);if(this.compareF(oldVal,tr))return 0;state.values[idx]=tr;return 1},reconfigure:(state,oldState)=>{if(null!=oldState.config.address[this.id])return state.values[idx]=oldState.field(this),0;state.values[idx]=this.create(state);return 1}}}init(create){return[this,initField.of({field:this,create})]}get extension(){return this}}const Prec_={lowest:4,low:3,
default:2,high:1,highest:0};global={highest:prec(Prec_.highest),high:prec(Prec_.high),default:prec(Prec_.default),low:prec(Prec_.low),lowest:prec(Prec_.lowest)};class PrecExtension{constructor(inner,prec){this.inner=inner;this.prec=prec}}class Compartment{of(ext){return new CompartmentInstance(this,ext)}reconfigure(content){return Compartment.reconfigure.of({compartment:this,extension:content})}get(state){return state.config.compartments.get(this)}}class CompartmentInstance{constructor(compartment,
inner){this.compartment=compartment;this.inner=inner}}class Configuration{constructor(base,compartments,dynamicSlots,address,staticValues,facets){this.base=base;this.compartments=compartments;this.dynamicSlots=dynamicSlots;this.address=address;this.staticValues=staticValues;this.facets=facets;for(this.statusTemplate=[];this.statusTemplate.length<dynamicSlots.length;)this.statusTemplate.push(0)}staticFacet(facet){let addr=this.address[facet.id];return null==addr?facet.default:this.staticValues[addr>>
1]}static resolve(base,compartments,oldState){var fields=[];let facets=Object.create(null),newCompartments=new Map;for(var ext of flatten(base,compartments,newCompartments))ext instanceof StateField?fields.push(ext):(facets[ext.facet.id]||(facets[ext.facet.id]=[])).push(ext);let address=Object.create(null);compartments=[];ext=[];for(let field of fields)address[field.id]=ext.length<<1,ext.push(a=>field.slot(a));fields=null===oldState||void 0===oldState?void 0:oldState.config.facets;for(let id in facets){let providers=
facets[id],facet=providers[0].facet;var oldProviders=fields&&fields[id]||[];if(providers.every(p=>0==p.type))address[facet.id]=compartments.length<<1|1,sameArray(oldProviders,providers)?compartments.push(oldState.facet(facet)):(oldProviders=facet.combine(providers.map(p=>p.value)),compartments.push(oldState&&facet.compare(oldProviders,oldState.facet(facet))?oldState.facet(facet):oldProviders));else{for(let p of providers)0==p.type?(address[p.id]=compartments.length<<1|1,compartments.push(p.value)):
(address[p.id]=ext.length<<1,ext.push(a=>p.dynamicSlot(a)));address[facet.id]=ext.length<<1;ext.push(a=>dynamicFacetSlot(a,facet,providers))}}oldState=ext.map(f=>f(address));return new Configuration(base,newCompartments,oldState,address,compartments,facets)}}const languageData=Facet.define(),allowMultipleSelections=Facet.define({combine:values=>values.some(v=>v),static:!0}),lineSeparator=Facet.define({combine:values=>values.length?values[0]:void 0,static:!0}),changeFilter=Facet.define(),transactionFilter=
Facet.define(),transactionExtender=Facet.define(),readOnly=Facet.define({combine:values=>values.length?values[0]:!1});class Annotation{constructor(type,value){this.type=type;this.value=value}static define(){return new AnnotationType}}class AnnotationType{of(value){return new Annotation(this,value)}}class StateEffectType{constructor(map){this.map=map}of(value){return new StateEffect(this,value)}}class StateEffect{constructor(type,value){this.type=type;this.value=value}map(mapping){mapping=this.type.map(this.value,
...
fromB,length)=>compare(sideA,fromA,sideB,fromB,length,comparator));textDiff.empty&&0==textDiff.length&&compare(sideA,0,sideB,0,0,comparator)}static eq(oldSets,newSets,from=0,to){null==to&&(to=1E9-1);var a=oldSets.filter(set=>!set.isEmpty&&0>newSets.indexOf(set));let b=newSets.filter(set=>!set.isEmpty&&0>oldSets.indexOf(set));if(a.length!=b.length)return!1;if(!a.length)return!0;let sharedChunks=findSharedChunks(a,b);a=(new SpanCursor(a,sharedChunks,0)).goto(from);for(from=(new SpanCursor(b,sharedChunks,
0)).goto(from);;){if(a.to!=from.to||!sameValues(a.active,from.active)||a.point&&(!from.point||!a.point.eq(from.point)))return!1;if(a.to>to)return!0;a.next();from.next()}}static spans(sets,from,to,iterator,minPointSize=-1){sets=(new SpanCursor(sets,null,minPointSize)).goto(from);minPointSize=from;let openRanges=sets.openStart;for(;;){let curTo=Math.min(sets.to,to);if(sets.point){let active=sets.activeForPoint(sets.to);iterator.point(minPointSize,curTo,sets.point,active,sets.pointFrom<from?active.length+
1:Math.min(active.length,openRanges),sets.pointRank);openRanges=Math.min(sets.openEnd(curTo),active.length)}else curTo>minPointSize&&(iterator.span(minPointSize,curTo,sets.active,openRanges),openRanges=sets.openEnd(curTo));if(sets.to>to)return openRanges+(sets.point&&sets.to>to?1:0);minPointSize=sets.to;sets.next()}}static of(ranges,sort=!1){let build=new RangeSetBuilder;for(let range of ranges instanceof Range?[ranges]:sort?lazySort(ranges):ranges)build.add(range.from,range.to,range.value);return build.finish()}}
RangeSet.empty=new RangeSet([],[],null,-1);RangeSet.empty.nextLayer=RangeSet.empty;class RangeSetBuilder{constructor(){this.chunks=[];this.chunkPos=[];this.chunkStart=-1;this.last=null;this.lastTo=this.lastFrom=-1E9;this.from=[];this.to=[];this.value=[];this.setMaxPoint=this.maxPoint=-1;this.nextLayer=null}finishChunk(newArrays){this.chunks.push(new Chunk(this.from,this.to,this.value,this.maxPoint));this.chunkPos.push(this.chunkStart);this.chunkStart=-1;this.setMaxPoint=Math.max(this.setMaxPoint,
this.maxPoint);this.maxPoint=-1;newArrays&&(this.from=[],this.to=[],this.value=[])}add(from,to,value){this.addInner(from,to,value)||(this.nextLayer||(this.nextLayer=new RangeSetBuilder)).add(from,to,value)}addInner(from,to,value){let diff=from-this.lastTo||value.startSide-this.last.endSide;if(0>=diff&&0>(from-this.lastFrom||value.startSide-this.last.startSide))throw Error("Ranges must be added sorted by `from` position and `startSide`");if(0>diff)return!1;250==this.from.length&&this.finishChunk(!0);
0>this.chunkStart&&(this.chunkStart=from);this.from.push(from-this.chunkStart);this.to.push(to-this.chunkStart);this.last=value;this.lastFrom=from;this.lastTo=to;this.value.push(value);value.point&&(this.maxPoint=Math.max(this.maxPoint,to-from));return!0}addChunk(from,chunk){if(0>(from-this.lastTo||chunk.value[0].startSide-this.last.endSide))return!1;this.from.length&&this.finishChunk(!0);this.setMaxPoint=Math.max(this.setMaxPoint,chunk.maxPoint);this.chunks.push(chunk);this.chunkPos.push(from);let last=
chunk.value.length-1;this.last=chunk.value[last];this.lastFrom=chunk.from[last]+from;this.lastTo=chunk.to[last]+from;return!0}finish(){return this.finishInner(RangeSet.empty)}finishInner(next){this.from.length&&this.finishChunk(!1);if(0==this.chunks.length)return next;next=RangeSet.create(this.chunkPos,this.chunks,this.nextLayer?this.nextLayer.finishInner(next):next,this.setMaxPoint);this.from=null;return next}}class LayerCursor{constructor(layer,skip,minPoint,rank=0){this.layer=layer;this.skip=skip;
this.minPoint=minPoint;this.rank=rank}get startSide(){return this.value?this.value.startSide:0}get endSide(){return this.value?this.value.endSide:0}goto(pos,side=-1E9){this.chunkIndex=this.rangeIndex=0;this.gotoInner(pos,side,!1);return this}gotoInner(pos,side,forward){for(;this.chunkIndex<this.layer.chunk.length;){let next=this.layer.chunk[this.chunkIndex];if(!(this.skip&&this.skip.has(next)||this.layer.chunkEnd(this.chunkIndex)<pos||next.maxPoint<this.minPoint))break;this.chunkIndex++;forward=!1}this.chunkIndex<
this.layer.chunk.length&&(pos=this.layer.chunk[this.chunkIndex].findIndex(pos-this.layer.chunkPos[this.chunkIndex],side,!0),(!forward||this.rangeIndex<pos)&&this.setRangeIndex(pos));this.next()}forward(pos,side){0>(this.to-pos||this.endSide-side)&&this.gotoInner(pos,side,!0)}next(){for(;;)if(this.chunkIndex==this.layer.chunk.length){this.from=this.to=1E9;this.value=null;break}else{let chunkPos=this.layer.chunkPos[this.chunkIndex],chunk=this.layer.chunk[this.chunkIndex];this.from=chunkPos+chunk.from[this.rangeIndex];
this.to=chunkPos+chunk.to[this.rangeIndex];this.value=chunk.value[this.rangeIndex];this.setRangeIndex(this.rangeIndex+1);if(0>this.minPoint||this.value.point&&this.to-this.from>=this.minPoint)break}}setRangeIndex(index){if(index==this.layer.chunk[this.chunkIndex].value.length){this.chunkIndex++;if(this.skip)for(;this.chunkIndex<this.layer.chunk.length&&this.skip.has(this.layer.chunk[this.chunkIndex]);)this.chunkIndex++;this.rangeIndex=0}else this.rangeIndex=index}nextChunk(){this.chunkIndex++;this.rangeIndex=
0;this.next()}compare(other){return this.from-other.from||this.startSide-other.startSide||this.rank-other.rank||this.to-other.to||this.endSide-other.endSide}}class HeapCursor{constructor(heap){this.heap=heap}static from(sets,skip=null,minPoint=-1){let heap=[];for(let i=0;i<sets.length;i++)for(let cur=sets[i];!cur.isEmpty;cur=cur.nextLayer)cur.maxPoint>=minPoint&&heap.push(new LayerCursor(cur,skip,minPoint,i));return 1==heap.length?heap[0]:new HeapCursor(heap)}get startSide(){return this.value?this.value.startSide:
0}goto(pos,side=-1E9){for(let cur of this.heap)cur.goto(pos,side);for(pos=this.heap.length>>1;0<=pos;pos--)heapBubble(this.heap,pos);this.next();return this}forward(pos,side){for(var cur of this.heap)cur.forward(pos,side);for(cur=this.heap.length>>1;0<=cur;cur--)heapBubble(this.heap,cur);0>(this.to-pos||this.value.endSide-side)&&this.next()}next(){if(0==this.heap.length)this.from=this.to=1E9,this.value=null,this.rank=-1;else{let top=this.heap[0];this.from=top.from;this.to=top.to;this.value=top.value;
this.rank=top.rank;top.value&&top.next();heapBubble(this.heap,0)}}}class SpanCursor{constructor(sets,skip,minPoint){this.minPoint=minPoint;this.active=[];this.activeTo=[];this.activeRank=[];this.minActive=-1;this.point=null;this.pointRank=this.pointFrom=0;this.to=-1E9;this.endSide=0;this.openStart=-1;this.cursor=HeapCursor.from(sets,skip,minPoint)}goto(pos,side=-1E9){this.cursor.goto(pos,side);this.active.length=this.activeTo.length=this.activeRank.length=0;this.minActive=-1;this.to=pos;this.endSide=
side;this.openStart=-1;this.next();return this}forward(pos,side){for(;-1<this.minActive&&0>(this.activeTo[this.minActive]-pos||this.active[this.minActive].endSide-side);)this.removeActive(this.minActive);this.cursor.forward(pos,side)}removeActive(index){remove(this.active,index);remove(this.activeTo,index);remove(this.activeRank,index);this.minActive=findMinIndex(this.active,this.activeTo)}addActive(trackOpen){let i=0,{value,to,rank}=this.cursor;for(;i<this.activeRank.length&&this.activeRank[i]<=
rank;)i++;insert(this.active,i,value);insert(this.activeTo,i,to);insert(this.activeRank,i,rank);trackOpen&&insert(trackOpen,i,this.cursor.from);this.minActive=findMinIndex(this.active,this.activeTo)}next(){let from=this.to;var wasPoint=this.point;this.point=null;let trackOpen=0>this.openStart?[]:null;for(;;){var a=this.minActive;if(-1<a&&0>(this.activeTo[a]-this.cursor.from||this.active[a].endSide-this.cursor.startSide)){if(this.activeTo[a]>from){this.to=this.activeTo[a];this.endSide=this.active[a].endSide;
break}this.removeActive(a);trackOpen&&remove(trackOpen,a)}else if(this.cursor.value)if(this.cursor.from>from){this.to=this.cursor.from;this.endSide=this.cursor.startSide;break}else if(a=this.cursor.value,a.point)if(wasPoint&&this.cursor.to==this.to&&this.cursor.from<this.cursor.to)this.cursor.next();else{this.point=a;this.pointFrom=this.cursor.from;this.pointRank=this.cursor.rank;this.to=this.cursor.to;this.endSide=a.endSide;this.cursor.next();this.forward(this.to,this.endSide);break}else this.addActive(trackOpen),
this.cursor.next();else{this.to=this.endSide=1E9;break}}if(trackOpen)for(this.openStart=0,wasPoint=trackOpen.length-1;0<=wasPoint&&trackOpen[wasPoint]<from;wasPoint--)this.openStart++}activeForPoint(to){if(!this.active.length)return this.active;let active=[];for(let i=this.active.length-1;0<=i&&!(this.activeRank[i]<this.pointRank);i--)(this.activeTo[i]>to||this.activeTo[i]==to&&this.active[i].endSide>=this.point.endSide)&&active.push(this.active[i]);return active.reverse()}openEnd(to){let open=0;
for(let i=this.activeTo.length-1;0<=i&&this.activeTo[i]>to;i--)open++;return open}}exports.Annotation=Annotation;exports.AnnotationType=AnnotationType;exports.ChangeDesc=ChangeDesc;exports.ChangeSet=ChangeSet;exports.Compartment=Compartment;exports.EditorSelection=EditorSelection;exports.EditorState=EditorState;exports.Facet=Facet;exports.Line=Line;exports.Prec=global;exports.Range=Range;exports.RangeSet=RangeSet;exports.RangeSetBuilder=RangeSetBuilder;exports.RangeValue=RangeValue;exports.SelectionRange=
SelectionRange;exports.StateEffect=StateEffect;exports.StateEffectType=StateEffectType;exports.StateField=StateField;exports.Text=Text;exports.Transaction=Transaction;exports.codePointAt=codePointAt;exports.codePointSize=codePointSize;exports.combineConfig=function(configs,defaults,combine={}){let result={};for(let config of configs)for(let key of Object.keys(config)){configs=config[key];let current=result[key];if(void 0===current)result[key]=configs;else if(current!==configs&&void 0!==configs)if(Object.hasOwnProperty.call(combine,
key))result[key]=combine[key](current,configs);else throw Error("Config merge conflict for field "+key);}for(let key in defaults)void 0===result[key]&&(result[key]=defaults[key]);return result};exports.countColumn=function(string,tabSize,to=string.length){let n=0;for(let i=0;i<to;)9==string.charCodeAt(i)?(n+=tabSize-n%tabSize,i++):(n++,i=findClusterBreak(string,i));return n};exports.findClusterBreak=findClusterBreak;exports.findColumn=function(string,col,tabSize,strict){for(let i=0,n=0;;){if(n>=col)return i;
if(i==string.length)break;n+=9==string.charCodeAt(i)?tabSize-n%tabSize:1;i=findClusterBreak(string,i)}return!0===strict?-1:string.length};exports.fromCodePoint=function(code){if(65535>=code)return String.fromCharCode(code);code-=65536;return String.fromCharCode((code>>10)+55296,(code&1023)+56320)}}
