...
head.bidiLevel||void 0)});if(selection.eq(view.state.selection))return!1;view.dispatch(setSel(view.state,selection));return!0}function selectByChar(view,forward){return extendSel(view,range=>view.moveByChar(range,forward))}function selectByGroup(view,forward){return extendSel(view,range=>view.moveByGroup(range,forward))}function selectBySubword(view,forward){return extendSel(view,range=>moveBySubword(view,range,forward))}function selectByLine(view,forward){return extendSel(view,range=>view.moveVertically(range,
forward))}function selectByPage(view,forward){return extendSel(view,range=>view.moveVertically(range,forward,Math.max(view.defaultLineHeight,Math.min(view.dom.clientHeight,innerHeight)-5)))}function deleteBy(target,by){if(target.state.readOnly)return!1;let event="delete.selection",{state:state$1}=target,changes=state$1.changeByRange(range=>{let {from,to}=range;if(from==to){let towards=by(from);towards<from?(event="delete.backward",towards=skipAtomic(target,towards,!1)):towards>from&&(event="delete.forward",
towards=skipAtomic(target,towards,!0));from=Math.min(from,towards);to=Math.max(to,towards)}else from=skipAtomic(target,from,!1),to=skipAtomic(target,to,!0);return from==to?{range}:{changes:{from,to},range:state$jscomp$0.EditorSelection.cursor(from)}});if(changes.changes.empty)return!1;target.dispatch(state$1.update(changes,{scrollIntoView:!0,userEvent:event,effects:"delete.selection"==event?view$jscomp$0.EditorView.announce.of(state$1.phrase("Selection deleted")):void 0}));return!0}function skipAtomic(target,
pos,forward){if(target instanceof view$jscomp$0.EditorView)for(let ranges of target.state.facet(view$jscomp$0.EditorView.atomicRanges).map(f=>f(target)))ranges.between(pos,pos,(from,to)=>{from<pos&&to>pos&&(pos=forward?to:from)});return pos}function selectedLineBlocks(state){let blocks=[];var upto=-1;for(let range of state.selection.ranges){let startLine=state.doc.lineAt(range.from),endLine=state.doc.lineAt(range.to);range.empty||range.to!=endLine.from||(endLine=state.doc.lineAt(range.to-1));upto>=
startLine.number?(upto=blocks[blocks.length-1],upto.to=endLine.to,upto.ranges.push(range)):blocks.push({from:startLine.from,to:endLine.to,ranges:[range]});upto=endLine.number+1}return blocks}function moveLine(state$1,dispatch,forward){if(state$1.readOnly)return!1;let changes=[],ranges=[];for(let block of selectedLineBlocks(state$1)){if(forward?block.to==state$1.doc.length:0==block.from)continue;let nextLine=state$1.doc.lineAt(forward?block.to+1:block.from-1),size=nextLine.length+1;if(forward){changes.push({from:block.to,
to:nextLine.to},{from:block.from,insert:nextLine.text+state$1.lineBreak});for(let r of block.ranges)ranges.push(state$jscomp$0.EditorSelection.range(Math.min(state$1.doc.length,r.anchor+size),Math.min(state$1.doc.length,r.head+size)))}else{changes.push({from:nextLine.from,to:block.from},{from:block.to,insert:state$1.lineBreak+nextLine.text});for(let r of block.ranges)ranges.push(state$jscomp$0.EditorSelection.range(r.anchor-size,r.head-size))}}if(!changes.length)return!1;dispatch(state$1.update({changes,
scrollIntoView:!0,selection:state$jscomp$0.EditorSelection.create(ranges,state$1.selection.mainIndex),userEvent:"move.line"}));return!0}function copyLine(state,dispatch,forward){if(state.readOnly)return!1;let changes=[];for(let block of selectedLineBlocks(state))forward?changes.push({from:block.from,insert:state.doc.slice(block.from,block.to)+state.lineBreak}):changes.push({from:block.to,insert:state.lineBreak+state.doc.slice(block.from,block.to)});dispatch(state.update({changes,scrollIntoView:!0,
userEvent:"input.copyline"}));return!0}function newlineAndIndent(atEof){return({state:state$1,dispatch})=>{if(state$1.readOnly)return!1;let changes=state$1.changeByRange(range=>{let {from,to}=range;range=state$1.doc.lineAt(from);var JSCompiler_temp;if(JSCompiler_temp=!atEof&&from==to)if(JSCompiler_temp=from,/\(\)|\[\]|\{\}/.test(state$1.sliceDoc(JSCompiler_temp-1,JSCompiler_temp+1)))JSCompiler_temp={from:JSCompiler_temp,to:JSCompiler_temp};else{var context=language.syntaxTree(state$1).resolveInner(JSCompiler_temp),
before=context.childBefore(JSCompiler_temp);context=context.childAfter(JSCompiler_temp);var closedBy;JSCompiler_temp=before&&context&&before.to<=JSCompiler_temp&&context.from>=JSCompiler_temp&&(closedBy=before.type.prop(common.NodeProp.closedBy))&&-1<closedBy.indexOf(context.name)&&state$1.doc.lineAt(before.to).from==state$1.doc.lineAt(context.from).from?{from:before.to,to:context.from}:null}closedBy=JSCompiler_temp;atEof&&(from=to=(to<=range.to?range:state$1.doc.lineAt(to)).to);before=new language.IndentContext(state$1,
{simulateBreak:from,simulateDoubleBreak:!!closedBy});JSCompiler_temp=language.getIndentation(before,from);null==JSCompiler_temp&&(JSCompiler_temp=/^\s*/.exec(state$1.doc.lineAt(from).text)[0].length);for(;to<range.to&&/\s/.test(range.text[to-range.from]);)to++;closedBy?{from,to}=closedBy:from>range.from&&from<range.from+100&&!/\S/.test(range.text.slice(0,from))&&(from=range.from);JSCompiler_temp=["",language.indentString(state$1,JSCompiler_temp)];closedBy&&JSCompiler_temp.push(language.indentString(state$1,
before.lineIndent(range.from,-1)));return{changes:{from,to,insert:state$jscomp$0.Text.of(JSCompiler_temp)},range:state$jscomp$0.EditorSelection.cursor(from+1+JSCompiler_temp[1].length)}});dispatch(state$1.update(changes,{scrollIntoView:!0,userEvent:"input"}));return!0}}function changeBySelectedLine(state$1,f){let atLine=-1;return state$1.changeByRange(range=>{let changes=[];for(var pos=range.from;pos<=range.to;)pos=state$1.doc.lineAt(pos),pos.number>atLine&&(range.empty||range.to>pos.from)&&(f(pos,
changes,range),atLine=pos.number),pos=pos.to+1;pos=state$1.changes(changes);return{changes,range:state$jscomp$0.EditorSelection.range(pos.mapPos(range.anchor,1),pos.mapPos(range.head,1))}})}Object.defineProperty(exports,"__esModule",{value:!0});var state$jscomp$0=require("module$node_modules$$codemirror$state$dist$index_cjs"),view$jscomp$0=require("module$node_modules$$codemirror$view$dist$index_cjs"),language=require("module$node_modules$$codemirror$language$dist$index_cjs"),common=require("module$node_modules$$lezer$common$dist$index_cjs");
const toggleComment=target=>{let config=getConfig(target.state);return config.line?toggleLineComment(target):config.block?toggleBlockCommentByLine(target):!1},toggleLineComment=command$jscomp$0(changeLineComment,0),lineComment=command$jscomp$0(changeLineComment,1),lineUncomment=command$jscomp$0(changeLineComment,2),toggleBlockComment=command$jscomp$0(changeBlockComment,0),blockComment=command$jscomp$0(changeBlockComment,1),blockUncomment=command$jscomp$0(changeBlockComment,2),toggleBlockCommentByLine=
command$jscomp$0((o,s)=>{let ranges=[];for(let r of s.selection.ranges){let fromLine=s.doc.lineAt(r.from),toLine=r.to<=fromLine.to?fromLine:s.doc.lineAt(r.to),last=ranges.length-1;0<=last&&ranges[last].to>fromLine.from?ranges[last].to=toLine.to:ranges.push({from:fromLine.from,to:toLine.to})}return changeBlockComment(o,s,ranges)},0),fromHistory=state$jscomp$0.Annotation.define(),isolateHistory=state$jscomp$0.Annotation.define(),invertedEffects=state$jscomp$0.Facet.define(),historyConfig=state$jscomp$0.Facet.define({combine(configs){return state$jscomp$0.combineConfig(configs,
